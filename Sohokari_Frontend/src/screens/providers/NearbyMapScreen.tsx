...
export default function NearbyMapScreen() {
  ...
  const { data: providers, isLoading } = useQuery({
    ...
    queryFn:  () => providersApi.getNearby({ lat: region.latitude, lng: region.longitude, radius: 10, category: category ?? undefined }),
    staleTime: 60_000,
  });
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {(providers ?? []).map((provider: ProviderSummaryResponse) => (
          <Marker
            key={provider.providerId}
            coordinate={{ latitude: region.latitude, longitude: region.longitude }}
            pinColor={provider.isAvailable ? (CATEGORY_COLORS[provider.serviceCategory] ?? Colors.primary) : Colors.textMuted}
          >
            <Callout onPress={() => navigation.navigate('ProviderProfile', { providerId: provider.providerId })}>
              <View style={styles.callout}>
                ...
                {provider.hourlyRate != null && <Text style={styles.calloutRate}>৳{provider.hourlyRate}/hr</Text>}
      ...
      <TouchableOpacity style={styles.myLocBtn} onPress={getMyLocation} disabled={locating}>
...
const styles = StyleSheet.create({
  ...
  countBadge:     { position: 'absolute', top: 68, alignSelf: 'center', backgroundColor: Colors.primary, borderRadius: 20, paddingHorizontal: 14, paddingVertical: 6, elevation: 4 },
  countText:      { fontSize: 12, color: Colors.white, fontWeight: '600' },
  loadingOverlay: { position: 'absolute', top: 68, alignSelf: 'center', backgroundColor: Colors.surface, borderRadius: 20, padding: 10, elevation: 4 },
  myLocBtn:       { position: 'absolute', bottom: 100, right: 16, width: 48, height: 48, borderRadius: 24, backgroundColor: Colors.surface, alignItems: 'center', justifyContent: 'center', elevation: 6, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.15, shadowRadius: 6 },
  legend:         { position: 'absolute', bottom: 100, left: 16, backgroundColor: Colors.surface, borderRadius: 10, padding: 10, gap: 6, elevation: 4 },
