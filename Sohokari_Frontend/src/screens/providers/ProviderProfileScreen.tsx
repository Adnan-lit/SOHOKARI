...
export default function ProviderProfileScreen() {
  ...
  const totalJobs = reputation?.totalCompleted ?? provider.totalCompletedBookings ?? 0;
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.statsRow}>
        <View style={styles.stat}>
          <Text style={styles.statVal}>{avgScore > 0 ? avgScore.toFixed(1) : '—'}</Text>
          <View style={{ flexDirection: 'row', gap: 2 }}>
            {[1,2,3,4,5].map(s => (
              <Ionicons key={s} name="star" size={12} color={s <= Math.round(avgScore) ? Colors.warning : Colors.border} />
            ))}
          </View>
        ...
        <View style={styles.statDivider} />
        <View style={styles.stat}>
          <Text style={styles.statVal}>{provider.hourlyRate ? `৳${provider.hourlyRate}` : '—'}</Text>
      ...
      {provider.isAvailable && (
        <View style={styles.bookWrap}>
          <Button title="Book This Provider" onPress={() => navigation.navigate('CreateBooking', { providerId: provider.providerId })} style={{ flex: 1 }} />
        </View>
      )}
          ...
          <View style={styles.skillRow}>
            {provider.nidVerified && (
              <View style={[styles.skillChip, { backgroundColor: '#E1F5EE' }]}>
                <Text style={[styles.skillText, { color: Colors.success }]}>✓ NID Verified</Text>
              </View>
            )}
            {provider.tradeLicenseVerified && (
              <View style={[styles.skillChip, { backgroundColor: '#E1F5EE' }]}>
                <Text style={[styles.skillText, { color: Colors.success }]}>✓ Trade License</Text>
              </View>
            )}
          </View>
        </View>
      ...
      <View style={styles.section}>
        ...
        {reviews && reviews.length > 0 ? (
          reviews.slice(0, 3).map((review: ReviewResponse) => (
            <View key={review.reviewId} style={styles.reviewCard}>
              <View style={styles.reviewHeader}>
                <Text style={styles.reviewAuthor}>{review.customerName}</Text>
                <View style={styles.reviewStars}>
                  {[1,2,3,4,5].map(s => (
                    <Ionicons key={s} name="star" size={12} color={s <= review.overallSatisfaction ? Colors.warning : Colors.border} />
                  ))}
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  skillChip: { backgroundColor: '#EBF0F8', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6 },
