...
export default function ReviewListScreen() {
  ...
  const renderItem = ({ item }: { item: Review }) => {
    const author = typeof item.customerId === 'object' ? item.customerId.name : 'Customer';
    ...
    return (
      <View style={styles.card}>
        ...
        <View style={styles.breakdown}>
          {(Object.keys(CRITERIA_LABELS) as (keyof Review)[]).map(key => {
      ...
      ListHeaderComponent={
        <View style={styles.summary}>
          ...
          <View style={styles.avgStars}>
            {[1,2,3,4,5].map(s => (
              <Ionicons key={s} name="star" size={20}
                color={s <= Math.round(Number(avg)) ? Colors.warning : Colors.border} />
            ))}
          </View>
